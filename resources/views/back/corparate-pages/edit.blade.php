            ...
            <div class="block-content">
                <div class="row justify-content-center">
                    <div class="col-md-10 col-lg-8">
                        <form action="{{route('admin.pages.update' , $corparatepage->id)}}" method="POST"  enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="one-ecom-product-name">Sayfa Adı</label>
                                <input type="text" class="form-control" id="one-ecom-product-name" name="name" value="{{$corparatepage->name}}" required>
                            </div>
                            <div class="form-group">
                                <label>Sayfa İçeriği</label>
                                <textarea id="js-ckeditor" name="content" required>{{$corparatepage->content}}</textarea>
                            </div>
                            <div class="form group row">
                                <label for="one-ecom-product-stock">Sayfa resim</label>
                                <input type="file" class="dropzone form-control" name="image" >
                                <img src="{{$corparatepage->image}}" style="width: 300px; height: 200px;" class="img-fluid img-thumbnail mt-3">
                            </div>
