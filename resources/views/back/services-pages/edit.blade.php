...
@endsection
@section('title' , 'Kurumsal Sayfa Düzenle')
@section('content')
            ...
            <div class="block-content">
                <div class="row justify-content-center">
                    <div class="col-md-10 col-lg-8">
                        <form action="{{route('admin.services-pages.update' , $servicespage->id)}}" method="POST"  enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="one-ecom-product-name">Sayfa Adı</label>
                                <input type="text" class="form-control" id="one-ecom-product-name" name="name" value="{{$servicespage->name}}" required>
                            </div>
                            <div class="form-group">
                                <label>Sayfa İçeriği</label>
                                <textarea id="js-ckeditor" name="content" required>{{$servicespage->content}}</textarea>
                            </div>
                            <div class="form group row">
                                <label for="one-ecom-product-stock">Sayfa resim</label>
                                <input type="file" class="dropzone form-control" name="image" >
                                <img src="{{$servicespage->image}}" style="width: 300px; height: 200px;" class="img-fluid img-thumbnail mt-3">
                            </div>
                            <div class="form group row mt-3">
                                <div class="col-lg-12">
                                    <button  type="submit" class="btn btn-success form-control">Düzenle</button>
                                </div>
                            </div>
                        </form>
                    </div>
